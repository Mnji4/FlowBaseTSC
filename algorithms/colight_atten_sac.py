...
from utils.attention_actor_agents import AttentionAgent
from utils.critics import AttentionCritic,ColightCritic
import numpy as np
...
class AttentionSAC(object):
    ...
    def step(self, obs, explore=False):
        ...
        for a_i in range(self.nagents):
            curr_5obs.append([observations[i] for i in cloest[a_i]])
    ...
    def update_critic(self, sample,cloest, soft=True, logger=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            batch_next_ac, batch_next_log_pi = pi(next_obs, return_log_pi=True)
    ...
    def update_policies(self, sample,cloest, soft=True, logger=None, **kwargs):
        ...
        for a_i in range(self.nagents):
            corrent_5obs.append(torch.stack([obs[i] for i in cloest[a_i]]))
            corrent_5acs.append(torch.stack([samp_acs.view(self.nagents,-1,8)[i] for i in cloest[a_i]]))
        critic_in = list(zip(torch.cat(corrent_5obs,dim = 1), torch.cat(corrent_5acs,dim = 1)))
