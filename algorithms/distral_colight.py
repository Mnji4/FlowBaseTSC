...
class Distral(object):
    ...
    def optimize_model(self, sample, policy):
        ...
        state_batch = critic_in#torch.Tensor(obs[0])
        ...
        next_state_values = self.target_critic(trgt_critic_in, return_all_q = True).gather(1, action_batch[0].argmax(dim = 1,keepdim = True))
