...
from utils.agents import GruAttentionAgent, AttentionAgent
from utils.critics import GruAttentionCritic, AttentionCritic
import numpy as np
MSELoss = torch.nn.MSELoss()

from copy import deepcopy
data = np.load('datahangzhou.npy', allow_pickle=True).item()
cloest = data['cloest']
mask = data['mask']
...
class AttentionSAC(object):
    ...
    def __init__(self, agent_init_params, sa_size,
        ...
        self.critic = GruAttentionCritic(sa_size[:5], hidden_dim=critic_hidden_dim,
                                      attend_heads=attend_heads)
        self.target_critic = GruAttentionCritic(sa_size[:5], hidden_dim=critic_hidden_dim,
                                             attend_heads=attend_heads)
    ...
    def step(self, observations, explore=False, training = True):
        """
        ...
        # act_h = self.central_agent.policy.h
        # act_h = act_h.view(self.nagents,-1,act_h.shape[1])
        ...
        corrent_5obs = []
        corrent_5acs = []
        tmp_obs = torch.tensor(np.zeros(observations[0].shape), dtype=torch.float32, device=observations[0].device)
        tmp_acs = torch.tensor(np.zeros(act[0].shape), dtype=torch.float32, device=act[0].device)
        for a_i in range(self.nagents):
            corrent_5obs.append(torch.stack([observations[i] if i != -1 else tmp_obs for i in cloest[a_i]]))
            corrent_5acs.append(torch.stack([act[i] if i != -1 else tmp_acs for i in cloest[a_i]]))
        critic_in = list(zip(torch.cat(corrent_5obs,dim = 1), torch.cat(corrent_5acs,dim = 1)))
        # 返回critic的每一步hidden
        _ = self.critic(critic_in)

        obs_h = self.critic.h
        obs_h = obs_h.view(self.nagents,-1,obs_h.shape[1])
        if training:
            return [act[i] for i in range(self.nagents)]
        else:
            return [act[i] for i in range(self.nagents)],obs_h.detach().numpy(), obs_h.detach().numpy() #act_h.detach().numpy(), obs_h.detach().numpy()
    ...
    def update_critic(self, sample,cloest, mask, soft=True, logger=None, **kwargs):
        ...
        obs, acs, rews, next_obs, dones, act_hidden_states, obs_hidden_states = sample
        # Q loss
        next_acs = []
        next_log_pis = []
        pi = self.central_agent.target_policy
        #pi.init_hidden_state(hidden_states = torch.cat(act_hidden_states))
        ...
        self.target_critic.init_hidden_state(hidden_states = torch.cat(obs_hidden_states))
        self.critic.init_hidden_state(hidden_states = torch.cat(obs_hidden_states))
    ...
    def update_policies(self, sample,cloest,mask,soft=True, logger=None, **kwargs):
        ...
        obs1 = torch.cat(obs)
        pi = self.central_agent.policy
        #pi.init_hidden_state(hidden_states = torch.cat(act_hidden_states))
        samp_acs,probs,log_pis,pol_regs =  pi(
    ...
    def prep_rollouts(self, device='cpu'):
        ...
        #self.central_agent.policy.init_hidden_state()
