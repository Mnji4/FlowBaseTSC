...
class Dqn(object):
    ...
    def __init__(self,
        ...
        self.critic = SingleCritic(
            ...
            norm_in=False)
        self.target_critic = SingleCritic(
            sa_size[:5],
            hidden_dim=critic_hidden_dim,
            attend_heads=attend_heads,
            norm_in=False)
        ...
        self.critic_optimizer = Adam(
            self.critic.parameters(), lr=q_lr, weight_decay=1e-3)
    ...
    def step(self, obs, explore=False):
        ...
        tmp_obs = torch.tensor(
            np.zeros(obs[0].shape), dtype=torch.float32, device=obs[0].device)
        for a_i in range(self.nagents):
            corrent_5obs.append(
                torch.stack([obs[i] if i == 0 else tmp_obs for i in range(5)]))
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        #next_5acs = []
        tmp_obs = torch.tensor(
            np.zeros(next_obs[0].shape),
            dtype=torch.float32,
            device=next_obs[0].device)
        #tmp_acs = torch.tensor(np.zeros(next_acs[0].shape), dtype=torch.float32, device=next_obs[0].device)
        next_mask_obs = []
        for a_i in range(self.nagents):
            next_5obs.append(
                torch.stack(
                    [next_obs[i] if i == 0 else tmp_obs for i in range(5)]))
        ...
        #local q loss
        target_q = (torch.cat(rews).view(-1, 1) + self.gamma * next_q *
