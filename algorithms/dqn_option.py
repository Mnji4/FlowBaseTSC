...
class DqnOption(object):
    """
    Wrapper class for SAC agents with central attention critic in multi-agent
    task
    """
    def __init__(self, agent_init_params, sa_size, n_option,
        ...
        self.critic = [SingleCritic(sa_size[:5], hidden_dim=critic_hidden_dim,
                                      attend_heads=attend_heads, norm_in = False) for _ in range(n_option)]
    ...
    @classmethod
    def init_from_env(cls, env, s_dim,a_dim, n_agent ,n_option, gamma=0.95, tau=0.01,
