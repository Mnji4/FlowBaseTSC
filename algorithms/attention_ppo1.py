...
class AttentionPPO(object):
    ...
    def update_policies(self, sample, soft=True, logger=None, **kwargs):
        ...
        for a_i, pi, ob in zip(range(self.nagents), self.target_policies, obs):
            ...
            curr_ac, log_pi, old_ent= pi(
                ob, return_log_pi=True, return_entropy=True)
            old_log_pis.append(log_pi)
            old_ents.append(old_ent)
        ...
        for a_i, probs, log_p ,old_log_p, old_ent,(q, all_q) in zip(range(self.nagents), all_probs,
                                                            all_log_pis,old_log_pis,old_ents,
                                                            critic_rets):
            curr_agent = self.agents[a_i]
            v = (all_q * probs).sum(dim=1, keepdim=True)
            advantages = (q - v).detach()
            ratio = torch.exp(log_p - old_log_p.detach())
            surr1 = ratio * advantages
            surr2 = torch.clamp(ratio, 1-self.eps_clip, 1+self.eps_clip) * advantages

            # final loss of clipped objective PPO
            pol_loss = -torch.min(surr1, surr2).mean() - 0.01*old_ent.mean()
    ...
    def update_all_targets(self):
        ...
        hard_update(self.central_agent.target_policy, self.central_agent.policy)
        self.init_from_central_agent()
    ...
    def save(self, filename):
        ...
        save_dict = {'init_dict': self.init_dict,
                     ...
                     'agent_params': [self.central_agent.get_params() for a in self.agents],
    ...
    def init_from_env(cls, env, gamma=0.95, tau=0.01,
        ...
        agent_init_params = [{'num_in_pol': 54,
                                       'num_out_pol': 4} for i in range(22)]
        sa_size = [(54,4) for i in range(22)]
