...
from utils.critics import MultiCritic
import numpy as np
...
class AttentionSAC(object):
    ...
    def __init__(self, agent_init_params, sa_size,
        ...
        self.critic = MultiCritic(sa_size, hidden_dim=critic_hidden_dim,
                                      attend_heads=attend_heads)
        self.target_critic = MultiCritic(sa_size, hidden_dim=critic_hidden_dim,
                                             attend_heads=attend_heads)
    ...
    def update_policies(self, sample,cloest, soft=True, logger=None, **kwargs):
        ...
        critic_in = list(zip(obs, samp_acs.view(859,-1,8)))
        critic_rets = self.critic(critic_in, return_all_q=True)

        q = []
        all_q = []
        for i in range(859):
            q.append(critic_rets[i][0])
            all_q.append(critic_rets[i][1])
        q = torch.cat(q)
