...
class QMixNet(nn.Module):
    def __init__(self, n_agents=0, obs_dim=0, hyper_hidden_dim=128, qmix_hidden_dim=128, two_hyper_layers=False):
        ...
        self.two_hyper_layers = two_hyper_layers
        ...
        if two_hyper_layers:
            self.hyper_w1 = nn.Sequential(nn.Linear(obs_dim, hyper_hidden_dim),
                                          nn.ReLU(),
                                          nn.Linear(hyper_hidden_dim, n_agents * qmix_hidden_dim))
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Sequential(nn.Linear(obs_dim, hyper_hidden_dim),
                                          nn.ReLU(),
                                          nn.Linear(hyper_hidden_dim, qmix_hidden_dim))
        else:
            self.hyper_w1 = nn.Linear(obs_dim, n_agents * qmix_hidden_dim)
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Linear(obs_dim, qmix_hidden_dim * 1)

        # hyper_w1得到的(经验条数，args.qmix_hidden_dim)矩阵需要同样维度的hyper_b1
        self.hyper_b1 = nn.Linear(obs_dim, qmix_hidden_dim)
        # hyper_w2得到的(经验条数，1)的矩阵需要同样维度的hyper_b1
        self.hyper_b2 =nn.Sequential(nn.Linear(obs_dim, qmix_hidden_dim),
                                     nn.ReLU(),
    ...
    def forward(self, q_values, states):  # states的shape为(episode_num, max_episode_len， state_shape)
        ...
        q_values = q_values.view(-1, 1, self.n_agents)  # (episode_num * max_episode_len, 1, n_agents) = (1920,1,5)
        states = states.reshape(-1, self.obs_dim)  # (episode_num * max_episode_len, state_shape)

        w1 = torch.abs(self.hyper_w1(states))  # (1920, 160)
        b1 = self.hyper_b1(states)  # (1920, 32)

        w1 = w1.view(-1, self.n_agents, self.qmix_hidden_dim)  # (1920, 5, 32)
        b1 = b1.view(-1, 1, self.qmix_hidden_dim)  # (1920, 1, 32)
        ...
        w2 = w2.view(-1, self.qmix_hidden_dim, 1)  # (1920, 32, 1)
