...
def epsilon_greedy(epsilon, probs, use_cuda=False):
    if torch.rand(1)>epsilon:
        int_acs = onehot_from_logits(torch.rand(probs.shape))
    else:
        int_acs = onehot_from_logits(probs)
    
    if use_cuda:
        int_acs = int_acs.cuda()
    else:
        tensor_type = torch.FloatTensor
    #acs = Variable(tensor_type(*probs.shape).fill_(0)).scatter_(1, int_acs, 1)
    return int_acs, int_acs
