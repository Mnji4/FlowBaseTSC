...
def run(config, start = 0):
    ...
    if config.load_model:
        for i in range(2):
            filename = Path(config.model_path + 'model%i_ep161.pt' %(i))
    ...
    for ep_i in range(1):
        ...
        for test_t in range(0,3600,config.interval_length):
            ...
            if 1:
                actions = [model[0].step([torch_obs[i]], explore=False)[0] for i in range(config.n_rollout_threads)][0]
            else:
        ...
        print(infos)
        r1_list.append(infos[0][0])
        r2_list.append(infos[0][2])
        ...
        for test_t in range(0,3600,config.interval_length):
            ...
            # get actions as torch Variables
            #[thread,agent,act]
            actions = [model[i].step([torch_obs[i]], explore=False)[0][0] for i in range(config.n_rollout_threads)]
    ...
    for j in range(len(r1_list)):    
        plt.plot([r1_list[j],0],[0,r2_list[j]])

    plt.plot([315,0],[0,2.34])
    plt.plot([370,0],[0,1.98])
    plt.savefig('132456.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path", default='/start/manhattan/models/Distral/test/run141/')
    parser.add_argument("--meta_model_path", default='manhattan/models/Distral/test/run55/meta_model.pt')
