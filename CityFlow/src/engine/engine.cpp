...
//                    assert(vehicle->getPartner() == nullptr);
    ...
    std::map<std::string, int> Engine::getLaneEffectiveVehicleCount(double range) const {
        std::map<std::string, int> ret;
        for (const Lane *lane : roadnet.getLanes()) {
            int cnt = 0;
            int lane_length = lane->getLength();
            for (Vehicle *vehicle : lane->getVehicles()) {
                if (lane_length - vehicle->getDistance() < range) { 
                    cnt += 1;
                }
            }
            ret.emplace(lane->getId(), cnt);
    ...
    std::map<std::string, int> Engine::getLaneEffectiveWaitingVehicleCount(double range) const {
        std::map<std::string, int> ret;
        for (const Lane *lane : roadnet.getLanes()) {
            int cnt = 0;
            int lane_length = lane->getLength();
            for (Vehicle *vehicle : lane->getVehicles()) {
                if ((vehicle->getSpeed() < 0.1) && (lane_length - vehicle->getDistance() < range)) { //TODO: better waiting critera
                    cnt += 1;
                }
            }
            ret.emplace(lane->getId(), cnt);
